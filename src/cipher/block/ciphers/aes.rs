...
impl BlockCipher<U16> for Aes {
    ...
    fn decrypt_block(&self, block: Block<U16>) -> Block<U16> {
        ...
        for r in (1..rounds).rev() {
            ...
            decrypted_block = self.inv_mix_cols(decrypted_block);
            decrypted_block = self.inv_shift_rows(decrypted_block);
            decrypted_block = self.inv_sub_bytes(decrypted_block);
        }
...
impl Aes {
    ...
    fn inv_sub_bytes(&self, input: Block<U16>) -> Block<U16> {
        ...
        for i in 0..input.len() {
            output[i] = INV_SBOX[input[i] as usize];
        }
    ...
    fn inv_mix_cols(&self, input: Block<U16>) -> Block<U16> {
        let mut output = GenericArray::default();
        ...
        for i in 0..4 {
            ...
            let mixed_col = self.inv_mix_col(col);
    ...
    fn inv_mix_col(&self, input: [u8; 4]) -> [u8; 4] {
        let mut output = [0u8; 4];

        output[0] = self.gf_mult(14, input[0])
            ^ self.gf_mult(11, input[1])
            ^ self.gf_mult(13, input[2])
            ^ self.gf_mult(9, input[3]);
        output[1] = self.gf_mult(9, input[0])
            ^ self.gf_mult(14, input[1])
            ^ self.gf_mult(11, input[2])
            ^ self.gf_mult(13, input[3]);
        output[2] = self.gf_mult(13, input[0])
            ^ self.gf_mult(9, input[1])
            ^ self.gf_mult(14, input[2])
            ^ self.gf_mult(11, input[3]);
        output[3] = self.gf_mult(11, input[0])
            ^ self.gf_mult(13, input[1])
            ^ self.gf_mult(9, input[2])
            ^ self.gf_mult(14, input[3]);
...
mod tests {
    ...
    fn test_key_sbox_q_c_one() {
        ...
        for i in 0..keys.len() {
            ...
            let aes_decrypt = Aes::new(keys[i].to_vec());
            let decrypted_block = aes_decrypt.decrypt_block(ciphertexts[i].into());
            assert_eq!(decrypted_block.as_slice(), plaintext)
        }
    }
    ...
    fn test_gf_sbox_appendix_b_one() {
        ...
        for i in 0..plaintexts.len() {
            ...
            let aes_decrypt = Aes::new(key.to_vec());
            let decrypted_block = aes_decrypt.decrypt_block(ciphertexts[i].into());
            assert_eq!(decrypted_block.as_slice(), &plaintexts[i])
        }
    }
    ...
    fn decrypt_block() {
        ...
        let ciphertext_block = GenericArray::clone_from_slice(&ciphertext);
        let decrypted_block = aes.decrypt_block(ciphertext_block);

        let expected_plaintext = b"ABCDEFGHIJKLMNOP";

        assert_eq!(decrypted_block.as_slice(), expected_plaintext);
    }
    ...
    #[test]
    fn test_varkey_appendix_e_three() {
        #[rustfmt::skip]
        let keys = [ // aes-256
